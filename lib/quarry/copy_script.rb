...
require 'quarry/constants'
require 'quarry/copy_script/context'
require 'quarry/copy_script/metadata'
require 'quarry/copy_script/commit'
#require 'quarry/copy_script/transaction'

module Quarry

  # Evaluation context for an ore's copy script.
  #
  class CopyScript

    #
    def self.run(mine, arguments, settings, options)
      new(mine, options).call(arguments, settings)
    end
    ...
    #
    # @param [Mine] mine
    #   Mine the CopyScript is going to processes.
    #
    def initialize(mine, options)
      @mine    = mine
      @options = options
    ...
    #   Mines can accept an Array of *arguments* which can refine their 
    #   behvior.
    #
    # @param [Hash] settings
    #   Mine can accept a Hash of `key=>value` *settings* which refine
    #   their behavior.
    #
    def call(arguments, settings)
      ...
      Dir.chdir(stage_directory) do
        scaffold do
          instance_eval(mine.script, mine.copyfile)
        end
      end
    end
    ...
    #def destname
    #  @destname ||= (
    #    File.basename((mine.options.output || Dir.pwd).chomp('/'))
    #  )
    #end
    ...
    #
    def resources
      [mine_settings, work_settings, user_settings]
    end

    #
    # Merged settings from mine_settings, work_settings and user_settings,
    ...
    def settings
      @settings ||= (
        sets = [user_settings, work_settings, mine_settings]
        sets.inject({}){ |h,s| h.merge!(s); h }
      )
    end

    #
    # Invocation settings are passed in via #initialize along with the mine.
    ...
    #
    def import(relpath)
      file = File.join(mine.path, relpath)
    ...
    def render(*args)
      ...
      [files].flatten.each do |fmatch|
        dir.glob_relative(fmatch).each do |f|
          next if f.basename.to_s == ".mine"  # MARKER
    ...
    #++
    #def plant(mine_name)
    #  #mine = Mine.new(name, mine.arguments, mine.settings)
    #  #Miner.new(mine, @options).quarry!(@stage)
    #end
    ...
    # This method acts as a precaution for mines that are 
    ...
    def ensure_empty(*ignore)
      ...
      if !files.empty? and !$FORCE
        raise "This mine is intended for new projects.\n" +
